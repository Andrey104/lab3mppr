...
class MatrixInput(object):
    def __init__(self, classifier):
        ...
        self.canvas = Canvas(self.root, bg="black", width=WIDTH * SIZE, height=HEIGHT * SIZE)
    ...
    def release(self, event):
        self.pressed = False
        prediction = self.callback(self.matrix.reshape(1, -1))
    ...
    def esc(self, event):
        data, _ = datasource.mnist.train.next_batch(1)
        for i in range(28):
            for j in range(28):
                self.matrix[i][j] = data[0][i * 28 + j]
        self.redraw()
        self.release(event)